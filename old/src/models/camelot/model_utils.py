...
def l_pred(y_true, y_pred, weights=None, name='pred_clus_loss'):
    ...
    if weights is None:
        weights = tf.cast(tf.constant(
            np.ones(shape=y_true.shape) / y_true.shape[-1]), dtype=np.float32)
    else:
        weights = tf.convert_to_tensor(value=weights, dtype="float32")

    # Compute batch and return
    batch_loss = - \
        tf.reduce_mean(tf.reduce_sum(weights * y_true *
                       log(y_pred), axis=-1), name=name)
...
def l_clus(clusters, name='emb_sep_L'):
    ...
    embedding_column = tf.expand_dims(
        clusters, axis=1)          # shape (K, 1, _)
    embedding_row = tf.expand_dims(
        clusters, axis=0)             # shape (1, K, _)

    # Compute L1 distance
    pairwise_loss = - \
        tf.reduce_sum((embedding_column - embedding_row)
                      ** 2, axis=-1)  # shape K, K
...
def l_dist(clus_prob, name="clus_sel_loss"):
    ...
    neg_entropy = tf.reduce_sum(
        multiply(avg_prob_per_clus, log(avg_prob_per_clus)))
...
class y_clus_cross_entropy(cbck.Callback):
    ...
    def on_epoch_end(self, epoch, logs={}):
        if epoch % self.interval == 0:
            ...
            clus_phenotypes = self.model.Predictor(
                self.model.embeddings).numpy() + epsilon

            # Iterate over all pairs of clusters and compute symmetric CE
            for i in range(K):
                for j in range(i + 1, K):
                    ce_sep += - \
                        np.sum(clus_phenotypes[i, :] *
                               np.log(clus_phenotypes[j, :]))
                    ce_sep += - \
                        np.sum(clus_phenotypes[j, :] *
                               np.log(clus_phenotypes[i, :]))

            # normalise and print output
            norm_loss = ce_sep / (K * (K + 1))
            print(
                "End of Epoch {:d} - CE sep : {:.4f}".format(epoch, norm_loss))
        ...
        if epoch % self.interval == 0:
            ...
            for true_class in range(self.C):
                for pred_class in range(self.C):
                    num_samples = np.logical_and(
                        y_pred == pred_class, y_true == true_class).sum()
                    cm_output[true_class, pred_class] = num_samples

            print(
                "End of Epoch {:d} - Confusion matrix: \n {}".format(epoch, cm_output.astype(int)))
...
class AUROC(cbck.Callback):
    ...
    def on_epoch_end(self, epoch, logs={}):
        if epoch % self.interval == 0:
            ...
            print(
                "End of Epoch {:d} - ROC score: {}".format(epoch, roc_auc_score))
...
class PrintClustersUsed(cbck.Callback):
    ...
    def on_epoch_end(self, epoch, logs={}):
        if epoch % self.interval == 0:
            ...
            clus_pred = self.model.Identifier(
                self.model.Encoder(self.X_val)).numpy()
            num_clusters = np.unique(np.argmax(clus_pred, axis=-1))
            avg_cluster_dist = np.mean(clus_pred, axis=-1)

            # Print Information
            print("End of Epoch {:d} - num_clusters {} - cluster dist {}".format(
                epoch, num_clusters, avg_cluster_dist))
...
class SupervisedTargetMetrics(cbck.Callback):
    ...
    def on_epoch_end(self, epoch, logs={}):
        if epoch % self.interval == 0:
            ...
            print(
                "End of Epoch {:d} - NMI {:.2f} , ARS {:.2f} , Purity {:.2f}".format(epoch, nmi, ars, purity))
    ...
    def on_epoch_end(self, epoch, logs={}):
        if epoch % self.interval == 0:
            ...
            sil = silhouette_score(
                X=X_val_2d, labels=y_pred, random_state=self.model.seed)
            sil_l = silhouette_score(
                X=self.latent_reps, labels=y_pred, random_state=self.model.seed)
...
def get_callbacks(track_loss, early_stop=True, lr_scheduler=True, tensorboard=True, min_delta=0.0001, patience=100):
    ...
    csv_logger = cbck.CSVLogger(
        filename=save_fd + "loss_tracker.csv", separator=",", append=True)
    ...
    if tensorboard:
        cbck_list.append(cbck.TensorBoard(
            log_dir=save_fd + "logs/", histogram_freq=1))
